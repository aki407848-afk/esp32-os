// 🛡️ ChiperOS v3.3 - Fixed Scope Errors + Bruce Menu + Animations
#include <Arduino.h>
#include <WiFi.h>
#include <WebServer.h>
#include <TFT_eSPI.h>
...
int irFreq = 30000;
const int irFreqMin = 30000, irFreqMax = 56000, irFreqStep = 500;int irPulseLen = 10;
bool irFwd = true;
...
void setup() {
  ...
  if (crashCount >= 3) enterSafeMode();  else prefs.putInt("crashes", crashCount + 1);
  ...
  Serial.println("✅ ChiperOS v3.3 Ready");
...
void handleButtons() {
  ...
  if (digitalRead(BTN_UP) == LOW || digitalRead(BTN_DOWN) == LOW || digitalRead(BTN_SEL) == LOW) {
    if (!btnHold) { lastPress = millis(); btnHold = true; holdStart = millis(); }
    if (digitalRead(BTN_UP) == LOW && digitalRead(BTN_DOWN) == LOW) {
      if (millis() - holdStart > 2000) { Serial.println("🚨 Emergency Combo"); enterSafeMode(); btnHold = false; }
      return;
    }
    if (millis() - lastPress > 250 && btnHold) {
      btnHold = false;
      if (digitalRead(BTN_UP) == LOW) { menuIdx = (menuIdx > 0) ? menuIdx - 1 : maxIdx; tftDrawMenu(); }
      else if (digitalRead(BTN_DOWN) == LOW) { menuIdx = (menuIdx < maxIdx) ? menuIdx + 1 : 0; tftDrawMenu(); }
      else if (digitalRead(BTN_SEL) == LOW) {
        if (currentMenu == MAIN) {
          if (menuIdx == 0) { currentMenu = WIFI_MENU; activeMenuPtr = wifiMenu; maxIdx = 3; }
          else if (menuIdx == 1) { currentMenu = IR_MENU; activeMenuPtr = irMenu; maxIdx = 3; }
          else if (menuIdx == 2) { currentMenu = BT_MENU; activeMenuPtr = btMenu; maxIdx = 2; }
          else if (menuIdx == 3) { currentMenu = SET_MENU; activeMenuPtr = setMenu; maxIdx = 2; }
          menuIdx = 0; tftDrawMenu();
        } else {
...
void tftDrawMenu() {
  ...
  for(int i=0; i<=maxIdx; i++) {
    int y = 30 + i*35;
    if(i == menuIdx) { tft.fillRect(5, y-2, 230, 30, TFT_DARKGREEN); tft.setTextColor(TFT_BLACK, TFT_DARKGREEN); }
...
void tftDrawAnim(const String& msg) {
  ...
  tft.fillRect(20, 120, 200, 8, TFT_DARKGREY);  for(int i=0; i<=200; i+=20) { tft.fillRect(20, 120, i, 8, TFT_GREEN); delay(40); }
  delay(600); tftDrawMenu();
}
...
void loop() {
  ...
  // FIX: comboStart declared here, visible in both if/else
  ...
  if (currentMode == WEB_MODE || inSafeMode) {
    ...
    if (digitalRead(BTN_UP) == LOW && digitalRead(BTN_DOWN) == LOW) {
      ...
      comboStart = 0; // Now correctly in scope
    }
  }
  ...
  if (!inSafeMode) {
    ...
    if (fakeApOn && millis()-lastFakeAp>200) { lastFakeAp=millis(); sendFakeAP(); }    if (bleOn && millis()-lastBle>100) {
      lastBle=millis();
    ...
    if (irGunOn && millis()-lastIr > irPulseLen) {
      ...
      if(irFwd) { irFreq += irFreqStep; if(irFreq >= irFreqMax) { irFreq = irFreqMax; irFwd = false; } }
      else { irFreq -= irFreqStep; if(irFreq <= irFreqMin) { irFreq = irFreqMin; irFwd = true; } }
    }
  }
  yield();
}
