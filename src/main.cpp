...
// ================= ВЕБ-ИНТЕРФЕЙС =================
const char* index_html = R"rawliteral(<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1">
...
function saveBeacons(){let json=document.getElementById('ssidlist').value;  fetch('/api/beacons',{method:'POST',headers:{'Content-Type':'application/json'},body:json})
...
void loadBeaconList() {
  File f = LittleFS.open("/beacons.json", "r");  if (!f) {
...
void handleUpload() {
  ...
  } else if (up.status == UPLOAD_FILE_END) {    if (up.filename.endsWith(".bin")) {
