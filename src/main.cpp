...
#define TFT_SCK  12
#define TFT_MOSI 13
#define BTN_NEXT -1  // Отключено (GPIO1 ломает загрузку)
#define BTN_SEL  -1  // Отключено (GPIO2 ломает загрузку)
...
// ================= ВЕБ-ИНТЕРФЕЙС =================
const char* index_html = R"HTML(<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1">
...
.bar{width:100%;height:8px;background:#334155;border-radius:4px;margin:10px 0;overflow:hidden;}
.bar div{height:100%;background:#22d3ee;width:0%;transition:width 0.2s;}
label{display:block;margin:10px 0 5px;color:#94a3b8;text-align:left;}
</style></head><body>
<div class="card"><h1>🛡️ ESP32-S3 OS v2</h1><div class="status" id="st">⏳ Загрузка...</div></div>
...
<div class="card"><h3>📝 Beacon SSID List</h3>
<label>Формат: {"ssids":["WiFi1","WiFi2"]}</label>
<textarea id="ssidlist" rows="3" placeholder='{"ssids":["Home_WiFi","Starbucks"]}'></textarea><br>
...
function saveBeacons(){let json=document.getElementById('ssidlist').value;fetch('/api/beacons',{method:'POST',headers:{'Content-Type':'application/json'},body:json}).then(r=>r.text()).then(t=>{document.getElementById('m').innerText=t;ref();});}
...
void setup() {
  ...
  server.on("/", []() { server.send(200, "text/html", index_html); });  server.on("/api/st", handleStatus);
...
void sendBeacon(const char* ssid) {
  uint8_t beacon[] = {0x80,0x00,0x00,0x00,0xff,0xff,0xff,0xff,0xff,0xff,0x01,0x02,0x03,0x04,0x05,0x06,0x01,0x02,0x03,0x04,0x05,0x06,0x00,0x00,0x83,0x51,0xf7,0x8f,0x0f,0x00,0x00,0x00,0x64,0x00,0x01,0x04,0x00,0x00};
  for(int i=4;i<10;i++) beacon[i]=esp_random()&0xFF;
  for(int i=10;i<16;i++) beacon[i]=beacon[i-6];
  int len=strlen(ssid); uint8_t full[64];
  memcpy(full, beacon, 16); full[16]=len; memcpy(&full[17], ssid, len);
...
void runAttacks() {
  ...
  if (beaconSpamOn) {
    static unsigned long lb=0; static int si=0;    if(now-lb>100){lb=now; if(beaconCount>0){sendBeacon(beaconSSIDs[si].c_str()); si=(si+1)%beaconCount;} yield();}
  }
