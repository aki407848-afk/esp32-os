...
// ================= ПИНЫ (БЕЗОПАСНЫЕ) =================
// Кнопки отключены (GPIO 1 и 2 мешают загрузке ESP32-S3!)
#define TFT_CS   14
#define TFT_DC   15
#define TFT_RST  16
#define TFT_BL   48
#define TFT_SCK  12
#define TFT_MOSI 13
#define BTN_NEXT -1  // Отключено!
#define BTN_SEL  -1  // Отключено!
...
// ================= ВЕБ-ИНТЕРФЕЙС =================const char* index_html = R"rawliteral(
...
<div class="card">
  <h3>📂 Загрузка файлов (OTA)</h3>
  <input type="file" id="f"><br>
  <button class="btn" onclick="up()">📤 Отправить .bin</button>
...
function cmd(m,a){fetch(`/api/cmd?m=${m}&a=${a}`).then(r=>r.text()).then(t=>{document.getElementById('m').innerText=t;ref();});}function saveBeacons(){let json=document.getElementById('ssidlist').value;
...
x.onload=()=>{document.getElementById('m').innerText=x.status==200?'✅ Готово! Перезагрузка...':'❌ Ошибка';ref();};
x.send(fd);}
...
void loop() {
  ...
  // Кнопки отключены
  // if (BTN_SEL >= 0 && digitalRead(BTN_SEL) == LOW) { 
  //   delay(200); 
  //   Serial.println("BTN_SEL"); 
  // }
  // if (BTN_NEXT >= 0 && digitalRead(BTN_NEXT) == LOW) { 
  //   delay(200); 
  //   Serial.println("BTN_NEXT"); 
  // }
}

// ================= ЗАГРУЗКА BEACON СПИСКА =================
void loadBeaconList() {
...
void runAttacks() {
  ...
  // WiFi Deauth - 5 каналов в секунду
  if (wifiDeauthOn) {
...
void handleBeaconsSave() {
  ...
  File f = LittleFS.open("/beacons.json", "w");  if (f) {
    f.print(json);
    f.close();
    loadBeaconList();
